...
function grafico(meses){
 ...
 const labels=pontos.map(p=>`<text x="${p.x}" y="166" class="avc-eixo">${p.rotulo}</text>`).join('');
...
function render(periodo=12){
 const tela=criar(),meses=dados(periodo),todos=meses.flatMap(m=>m.lista),mesAtual=meses[meses.length-1],listaAtual=mesAtual?.lista||[],total=mesAtual?.total||0,com=mesAtual?.comissao||0,media=todos.length/periodo;
 const melhor=meses.reduce((a,b)=>b.qtd>a.qtd?b:a,meses[0]);
 const anterior=meses[meses.length-2]?.qtd||0,atual=mesAtual?.qtd||0,crescimento=anterior?Math.round((atual-anterior)/anterior*100):(atual?100:0);
 const tipos=resumoTipos(listaAtual),maxTipo=Math.max(1,...tipos.map(t=>t.qtd));
 tela.innerHTML=`<div class="avc-cabecalho"><button type="button" class="avc-voltar" aria-label="Voltar">←</button><div><h1>📈 Evolução de vendas</h1><p>Análise completa do desempenho</p></div><select class="avc-periodo" aria-label="Período"><option value="6" ${periodo===6?'selected':''}>6 meses</option><option value="12" ${periodo===12?'selected':''}>12 meses</option></select></div><div class="avc-metricas"><article class="avc-metrica"><small>VENDAS NO MÊS ATUAL</small><strong>${atual}</strong><span>vendas neste mês</span></article><article class="avc-metrica"><small>MÉDIA / MÊS</small><strong>${media.toFixed(1).replace('.',',')}</strong><span>vendas por mês</span></article><article class="avc-metrica"><small>MÊS ATUAL</small><strong>${mesAtual?.mes?.split(' de ')[0]||'—'}</strong><span>${atual} ${atual===1?'venda':'vendas'}</span></article><article class="avc-metrica avc-faturamento"><small>FATURAMENTO DO MÊS</small><strong>${moeda(total)}</strong><span>Comissão: ${moeda(com)}</span></article></div><section class="avc-bloco"><div class="avc-titulo"><h2>EVOLUÇÃO MENSAL</h2><div class="avc-alternadores"><button type="button" data-modo="linha" class="${modo==='linha'?'ativo':''}">Linha</button><button type="button" data-modo="area" class="${modo==='area'?'ativo':''}">Área</button><button type="button" data-modo="barras" class="${modo==='barras'?'ativo':''}">Barras</button></div></div><div class="avc-grafico">${grafico(meses)}<div class="avc-dica"></div></div><p class="avc-instrucao">Toque nos pontos do gráfico para ver os detalhes do mês.</p></section><section class="avc-bloco"><div class="avc-titulo"><h2>INSIGHTS</h2></div><div class="avc-insights"><article class="avc-insight"><span>🚀</span><div><b>${crescimento>=0?'Crescimento':'Atenção ao desempenho'}</b><p>O mês atual está ${Math.abs(crescimento)}% ${crescimento>=0?'acima':'abaixo'} do mês anterior.</p></div></article><article class="avc-insight"><span>🏆</span><div><b>Melhor desempenho</b><p>${melhor?.mes||'Sem dados'} foi o melhor período, com ${melhor?.qtd||0} vendas.</p></div></article><article class="avc-insight"><span>💡</span><div><b>Tendência</b><p>${crescimento>=0?'A evolução recente indica uma tendência positiva.':'Vale acompanhar os próximos meses e reforçar as oportunidades.'}</p></div></article></div></section><section class="avc-bloco"><div class="avc-titulo"><h2>TIPOS DE NEGOCIAÇÃO — MÊS ATUAL</h2></div><div class="avc-tipos">${tipos.map(t=>`<div class="avc-tipo"><label>${t.nome}</label><strong>${t.qtd}</strong><div class="avc-progresso"><i style="--percentual:${(t.qtd/maxTipo)*100}%"></i></div></div>`).join('')}</div></section>`;
