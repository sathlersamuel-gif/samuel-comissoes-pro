(function(){
  ...
  const VERSION='guardia-profunda-3.0.0';
  ...
  const mutationObserver=new MutationObserver(records=>{
    mutationCount+=records.reduce((sum,record)=>sum+record.addedNodes.length+record.removedNodes.length,0);
  });
  mutationObserver.observe(document.documentElement,{childList:true,subtree:true});

  function runtimeErrorsCheck(){
    const errors=safeJson(localStorage.getItem(ERROR_KEY),[])||[];
  ...
  function navigationCheck(){
    ...
    const ok=visible(nav)&&buttons.length>=4&&buttons.every(button=>!button.disabled&&visible(button));
    return item('Navegação e botões principais',ok,ok?`Barra acessível com ${buttons.length} botões: ${labels}.`:'Barra inferior ausente, invisível ou com botão bloqueado.','critical','navigation');
  }
  ...
  function blockingLayerCheck(){
    ...
    const blockers=[...document.querySelectorAll('body *')].filter(el=>{
      ...
      const covers=rect.width>=innerWidth*.9&&rect.height>=innerHeight*.9;
      const z=Number.parseInt(style.zIndex,10)||0;
      return covers&&z>=1000&&style.pointerEvents!=='none';
    });
  ...
  function resourceCheck(){
    ...
    const failed=entries.filter(entry=>entry.name.startsWith(location.origin)&&entry.transferSize===0&&entry.duration>0&&!entry.name.includes('firebase'));
    return item('Arquivos carregados no aparelho',failed.length===0,failed.length?`${failed.length} recurso(s) podem ter falhado: ${failed.slice(0,5).map(entry=>entry.name.split('/').pop()).join(', ')}.`:'Nenhuma falha evidente no carregamento dos arquivos locais.','important','resources');
  }
  ...
  async function eventLoopCheck(){
    ...
    const lag=performance.now()-start-120;
    return item('Resposta e travamento do aplicativo',lag<450,lag<450?`Interface respondeu normalmente (${Math.max(0,lag).toFixed(0)} ms de atraso).`:`Interface apresentou atraso grave de ${lag.toFixed(0)} ms.`,'critical','event-loop');
  }
  ...
  async function serviceWorkerCheck(){
    ...
    try{
      ...
      return item('Atualização e funcionamento offline',Boolean(registration)&&controlled,registration?`Registrado; controle da página: ${controlled?'ativo':'ainda não assumido'}.`:'Service Worker não registrado.','important','service-worker');
    }catch(error){return item('Atualização e funcionamento offline',false,error.message,'important','service-worker');}
  }

  async function repositoryMapCheck(){
    try{
      const response=await fetch(`guardian-app-manifest.json?t=${Date.now()}`,{cache:'no-store'});
      if(!response.ok)throw new Error(`HTTP ${response.status}`);
      const manifest=await response.json();
      const files=Array.isArray(manifest.files)?manifest.files:[];
      const bootstrap=Boolean(manifest.bootstrap);
      return item('Conhecimento do repositório',files.length>=25&&!bootstrap,bootstrap?`Mapa ainda é inicial, com ${files.length} arquivos; precisa ser regenerado pelo workflow.`:`Mapa integral carregado com ${files.length} arquivos, ${manifest.summary?.symbols||0} símbolos e ${manifest.summary?.domIds||0} elementos de tela.`,'important','repository-map');
    }catch(error){return item('Conhecimento do repositório',false,`Não foi possível ler o mapa: ${error.message}`,'important','repository-map');}
  }
  ...
  async function firebaseCheck(){
    ...
    if(!navigator.onLine)return item('Login e Firebase',false,'Sem internet para confirmar o servidor.','warning','firebase');
    try{
      await window.firebase.firestore().collection('monitor_dispositivos').doc(window.SCPGuardian?.deviceId||localStorage.getItem('scp_guardian_device_id')||current.uid).set({
        uid:current.uid,email:current.email||'',auditoriaProfunda:VERSION,ultimaAuditoriaProfundaLocal:new Date().toISOString(),ultimaAuditoriaProfunda:window.firebase.firestore.FieldValue.serverTimestamp()
      },{merge:true});
      return item('Login e Firebase',true,'Autenticação e gravação de diagnóstico confirmadas.','critical','firebase');
    }catch(error){return item('Login e Firebase',false,`${error.code||'erro'}: ${error.message||error}`,'critical','firebase');}
  }
  ...
  async function run(forced=false){
    ...
    running=true;
    try{
      const checks=[structureCheck(),navigationCheck(),blockingLayerCheck(),guardianPermissionCheck(),dataCheck(),runtimeErrorsCheck(),resourceCheck(),mutationFloodCheck(),await eventLoopCheck(),await serviceWorkerCheck(),await repositoryMapCheck(),await firebaseCheck()];
      return await save(checks,forced);
  ...
  function start(){
    ...
    document.addEventListener('click',event=>{
      if(event.target?.closest?.('#scpSecurityButton,#scpCloseSecurity,.bottom button'))setTimeout(()=>run(true),900);
    },true);
