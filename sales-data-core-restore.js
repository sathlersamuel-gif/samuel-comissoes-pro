...
const restoreKey=uid=>`scp_restauracao_decidida:${uid}`;
...
const normalizeDeleted=list=>{
  ...
  (list||[]).forEach(x=>{
    ...
    if(!k)return;
    const old=m.get(k);
    if(!old||String(r.excluidaEm||'')>String(old.excluidaEm||''))m.set(k,{id:k,excluidaEm:r.excluidaEm||new Date().toISOString()});
  });
  return[...m.values()];
};
...
function markPending(mode='merge'){
  ...
  localStorage.setItem(queueKey(user.uid),JSON.stringify({pending:true,mode:finalMode,uid:user.uid,updatedAt:new Date().toISOString(),version:VERSION}));
  pending=true;
}
...
async function migrateLegacy(uid){
  ...
  legacySales.forEach(sale=>{
    ...
    operations.push({type:'set',ref:salesRef(uid).doc(id),data:{...sale,id,atualizadoEm:firebase.firestore.FieldValue.serverTimestamp()}});
  });
  legacyDeleted.forEach(item=>{
    operations.push({type:'set',ref:deletedRef(uid).doc(String(item.id)),data:{...item,id:String(item.id)}});
    operations.push({type:'delete',ref:salesRef(uid).doc(String(item.id))});
  });
  await commitOperations(operations);
  await ref.set({vendasSubcolecaoVersao:MIGRATION_VERSION,sincronizacaoVersao:VERSION,migradoEm:firebase.firestore.FieldValue.serverTimestamp(),legadoPreservado:true},{merge:true});
}
async function readCloud(uid){
  await migrateLegacy(uid);
  const [salesSnap,deletedSnap]=await Promise.all([salesRef(uid).get(),deletedRef(uid).get()]);
  return{sales:salesSnap.docs.map(d=>({...d.data(),id:d.id})),deleted:normalizeDeleted(deletedSnap.docs.map(d=>({...d.data(),id:d.id})))};
}
...
async function saveCloud(force=false){
  ...
  try{
    ...
    if(replace)await replaceCloud(uid,list);
    else{
      const operations=[];
      list.forEach(sale=>{
        const id=saleId(sale);
      ...
      del.forEach(item=>{
        ...
        operations.push({type:'set',ref:deletedRef(uid).doc(id),data:{...item,id}});
        operations.push({type:'delete',ref:salesRef(uid).doc(id)});
      });
      await commitOperations(operations);
    }
    await userRef(uid).set({email:user.email||'',vendasSubcolecaoVersao:MIGRATION_VERSION,sincronizacaoVersao:VERSION,atualizadoEm:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});
    localStorage.removeItem(queueKey(uid));
    pending=false;
  }catch(e){markPending();console.error('[Dados] Falha ao salvar:',e);}
  finally{
    saving=false;
    if(pending&&navigator.onLine&&user?.uid===uid)setTimeout(()=>saveCloud(true),800);
  }
}
function confirmRestore(count){
  return confirm(`Encontramos ${count} venda(s) salvas na sua conta.\n\nDeseja restaurar suas vendas e relatórios neste aparelho?\n\nCada venda continuará no mês da data original.`);
}
async function reconcile(options={}){
  ...
  try{
    ...
    const localDeleted=read(deletedKey(uid));
    const isFresh=options.fresh===true&&!local.length&&!localDeleted.length&&!localStorage.getItem(restoreKey(uid));
    if(isFresh&&cloud.sales.length){
      const restore=confirmRestore(cloud.sales.length);
      localStorage.setItem(restoreKey(uid),restore?'restaurado':'adiado');
      if(!restore){
        apply([],[]);
        return[];
      }
    }
    const del=normalizeDeleted([...localDeleted,...cloud.deleted]);
    localStorage.setItem(deletedKey(uid),JSON.stringify(del));
    const final=apply(merge(cloud.sales,local),del);
    markPending();
    await saveCloud(true);
    if(isFresh&&cloud.sales.length)alert(`${final.length} venda(s) restaurada(s) com sucesso. Os relatórios foram mantidos nos meses corretos.`);
    return final;
  }catch(e){markPending();console.error('[Dados] Falha ao reconciliar:',e);}
}
...
function installForm(){
  ...
  pct?.addEventListener('input',function(){this.value=percentText(this.value);try{if(typeof calcularComissao==='function')calcularComissao()}catch(_){}},true);
  document.getElementById('tipoVenda')?.addEventListener('change',()=>setTimeout(()=>{if(pct)try{if(typeof calcularComissao==='function')calcularComissao()}catch(_){}},0));
  form.addEventListener('submit',e=>{
    ...
    const sale={id:wasEditing?editId:Date.now(),cliente:get('cliente')?.value?.trim()||'',telefone:get('telefone')?.value||'',produto:get('produto')?.value?.trim()||'',tipo:get('tipoVenda')?.value||'',valor:v,porcentagem:p,comissao:v*p/100,data:get('dataVenda')?.value||new Date().toISOString().slice(0,10),observacao:get('observacao')?.value||''};
    let list=current();
    list=wasEditing?list.map(x=>saleId(x)===String(editId)?{...x,...sale,id:x.id}:x):[...list,sale];
    apply(list);save();delete form.dataset.editingId;form.reset();
    if(get('comissao'))get('comissao').value='';
    const btn=form.querySelector("button[type='submit']");if(btn)btn.textContent='SALVAR VENDA';
...
window.excluirVenda=async function(id){
  ...
  if(user)localStorage.setItem(deletedKey(user.uid),JSON.stringify(del));else localStorage.setItem(DELETED,JSON.stringify(del));
  apply(current().filter(v=>saleId(v)!==sid),del);
  if(user){markPending();await saveCloud(true)}
  return true;
};
window.editarVenda=function(id){
  const sale=current().find(v=>saleId(v)===String(id));if(!sale)return;
  const form=document.getElementById('formVenda');if(form)form.dataset.editingId=saleId(sale);
  ...
  set('cliente',sale.cliente);set('telefone',sale.telefone);set('produto',sale.produto||sale.modelo);set('tipoVenda',sale.tipo||sale.tipoVenda);
  set('valorVenda',Number(sale.valor||0).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2}));
  set('porcentagem',String(Number(sale.porcentagem||0).toFixed(1)).replace('.',','));set('dataVenda',sale.data);set('observacao',sale.observacao);
  const btn=document.querySelector("#formVenda button[type='submit']");if(btn)btn.textContent='SALVAR ALTERAÇÕES';
...
async function restoreBackup(list){
  ...
  const normalized=list.filter(v=>v&&typeof v==='object').map((v,i)=>({...v,id:v.id??Date.now()+i}));
  if(normalized.length!==list.length)throw new Error('Backup inválido');
  if(user)localStorage.setItem(deletedKey(user.uid),'[]');else localStorage.setItem(DELETED,'[]');
  apply(normalized,[]);
  if(user){localStorage.setItem(restoreKey(user.uid),'restaurado');markPending('replace');await saveCloud(true)}
  return normalized;
}
function installBackupImport(){
  const old=document.getElementById('importarBackup');if(!old||old.dataset.dataCore)return;
  const button=old.cloneNode(true);button.dataset.dataCore='1';old.replaceWith(button);
    ...
    input.addEventListener('change',()=>{
      ...
      reader.onload=async()=>{
        try{
          ...
          await restoreBackup(list);alert(`Backup importado com sucesso! ${list.length} venda(s) restaurada(s).`);
          try{if(typeof abrirTela==='function')abrirTela('dashboard')}catch(_){}
        }catch(e){console.error(e);alert('Não foi possível importar o backup. Selecione um arquivo JSON exportado pelo aplicativo.');}
      };
      reader.onerror=()=>alert('Não foi possível ler o arquivo selecionado.');reader.readAsText(file,'UTF-8');
    });
    input.click();
  });
}
async function onAuth(u){
  const run=++authRun;
  user=u||null;
  if(!u){lastUid='';pending=false;try{vendas=[]}catch(_){}localStorage.removeItem(STORAGE);localStorage.removeItem(OWNER);render();return;}
  if(lastUid&&lastUid!==u.uid){try{vendas=[]}catch(_){}localStorage.removeItem(STORAGE);}
  lastUid=u.uid;
  let own=read(userKey(u.uid));
  const legacyOwner=localStorage.getItem(OWNER);
  const legacy=read(STORAGE);
  if(!own.length&&legacy.length&&legacyOwner===u.uid){own=legacy;localStorage.setItem(userKey(u.uid),JSON.stringify(own));}
  const fresh=!own.length&&!read(deletedKey(u.uid)).length&&!localStorage.getItem(restoreKey(u.uid));
...
function install(){
  window.salvarBanco=save;installForm();installBackupImport();
  window.editarVendaSegura=id=>window.editarVenda(id);
  window.excluirVendaSegura=async function(id,ano,mes){const ok=await window.excluirVenda(id);if(ok&&typeof window.abrirMesAno==='function')window.abrirMesAno(ano,mes);};
  window.SCPDataCore={version:VERSION,restoreBackup,reconcile};
  const wait=()=>{if(!window.firebase||!firebase.apps?.length)return setTimeout(wait,100);firebase.auth().onAuthStateChanged(onAuth);};
  wait();
  window.addEventListener('online',()=>{if(user)reconcile()});
  window.addEventListener('scp:auth-ready',()=>{const u=firebase.auth().currentUser;if(u)onAuth(u);});
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden'&&user)saveCloud(true);});
  window.addEventListener('pagehide',()=>{if(user)saveCloud(true)});
  setInterval(()=>{if(user&&(pending||localStorage.getItem(queueKey(user.uid))))saveCloud(true);},5000);
