(function(){
  ...
  const CORE_VERSION='2026.07.18.1';
  let auth=null;
  let db=null;
  let processandoUid='';
  ...
  function botaoExcluir(alvo){
    return alvo?.closest?.('#listaUsuarios [data-acao="excluir"][data-uid],#listaUsuarios .btn-excluir-novo[data-uid]')||null;
  }

  function registrar(tipo,mensagem,dados){
    console[type==='erro'?'error':'log'](`[Gestão ${CORE_VERSION}] ${mensagem}`,dados||'');
  ...
  function marcarVersao(){
    ...
    if(!topo||document.getElementById('gestaoUsuariosVersao'))return;
    const selo=document.createElement('small');
    selo.id='gestaoUsuariosVersao';
    selo.textContent=`Gestão ${CORE_VERSION}`;
    selo.style.cssText='margin-left:auto;color:#647188;font-size:10px;font-weight:800;white-space:nowrap';
  ...
  async function excluirUsuario(botao){
    ...
    if(!confirm(`Tem certeza que deseja excluir ${email}?\n\nO usuário será removido da lista, os dados serão apagados e o acesso ficará bloqueado.`))return;
    ...
    try{
      ...
      const confirmado=await ref.get();
      if(!confirmado.exists||confirmado.data().status!==STATUS_EXCLUIDO){
        throw new Error('O Firebase não confirmou a exclusão.');
      }

      botao.closest('article,.usuario-card')?.remove();
      registrar('acao-validada','Usuário excluído e confirmado no Firebase',{uid,email});
      painelAberto();
      alert('Usuário excluído com sucesso.');
      setTimeout(filtrarExcluidos,100);
    }catch(erro){
  ...
  function instalarEventos(){
    ...
    document.addEventListener('click',evento=>{
      if(evento.target?.closest?.('#adminUsuariosBtn')){
        setTimeout(()=>{marcarVersao();filtrarExcluidos();},150);
        setTimeout(()=>{marcarVersao();filtrarExcluidos();},800);
      }
    },true);
    ...
    const observar=()=>{
      ...
      new MutationObserver(()=>{marcarVersao();setTimeout(filtrarExcluidos,50);}).observe(lista,{childList:true,subtree:true});
      marcarVersao();
      filtrarExcluidos();
    };
    observar();
    ...
    auth.onAuthStateChanged(async user=>{
      ...
      try{
        ...
        if(snap.exists&&snap.data().status===STATUS_EXCLUIDO){
          ...
          if(mensagem){mensagem.textContent='Acesso removido.';mensagem.style.color='#b00020';}
  ...
  function iniciar(){
    if(!iniciarFirebase())return setTimeout(iniciar,250);
    ...
    registrar('carregamento','Núcleo único de gerenciamento carregado');
  }
