...
function aplicarModoBeta(){
  document.body.classList.toggle('scp-beta-admin',ehAdmin());
}
...
function montarTopo(){
  ...
  if(!sino){
    ...
    sino.type='button';sino.id='scpSafeBell';
  ...
  if(!gear){
    ...
    gear.type='button';gear.id='scpSafeGear';
    gear.setAttribute('aria-label','Configurações');
    gear.textContent='⚙️';gear.onclick=abrirConfiguracoes;
  }
...
function iniciar(){
  estilos();
  let tentativas=0;
  const timer=setInterval(()=>{tentativas++;if(montarTopo()||tentativas>=40)clearInterval(timer)},150);
  const conectar=()=>firebase.auth().onAuthStateChanged(u=>{
    aplicarModoBeta();
    montarTopo();
    registrarVersao(u);
    acompanharPendentes();
  ...
  if(firebasePronto())conectar();
  else{
    let f=0;const aguarda=setInterval(()=>{f++;if(firebasePronto()){clearInterval(aguarda);conectar()}else if(f>=40)clearInterval(aguarda)},150);
  }
