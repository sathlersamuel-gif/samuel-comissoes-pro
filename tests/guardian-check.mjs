...
const gestorIphone = ler('user-management-iphone-fix.js');
...
verificar({id:'pwa-cache',arquivo:'sw.js / pwa-enhancements.js',descricao:'Cache e atualização PWA ativos',condicao:sw.includes('cachePrimeiro')&&sw.includes('redePrimeiro')&&index.includes('pwa-enhancements.js?v=14')&&pwa.includes("register('./sw.js?v=76'"),evidencia:{aprovada:'Estratégias de cache e Service Worker v76 localizados',falha:'Estratégia de cache ou versão atual do Service Worker ausente'},recomendacao:'Manter cachePrimeiro/redePrimeiro e registrar o Service Worker v76.'});
...
verificar({id:'usuarios-carregamento-unico',arquivo:'index.html / pwa-enhancements.js',descricao:'Gerenciamento moderno carregado uma única vez',condicao:index.includes('admin-access-settings-fix.js?v=2026.07.21.6')&&index.includes('user-management-modern-v2.js?v=2026.07.21.6')&&!pwa.includes('removerScripts')&&!pwa.includes('carregarGerenciamentoModerno'),evidencia:{aprovada:'Arquivos administrativos são carregados diretamente pelo HTML, sem recarregamento duplicado',falha:'O gerenciamento ainda pode ser executado duas vezes'},recomendacao:'Carregar os arquivos administrativos somente no index.html e não recriá-los pelo PWA.'});
...
verificar({id:'usuarios-iphone',arquivo:'pwa-enhancements.js / user-management-iphone-fix.js',descricao:'Correção específica do gerenciamento no iPhone carregada e protegida',condicao:pwa.includes('user-management-iphone-fix.js')&&gestorIphone.includes("const VERSION='2026.07.21.7'")&&gestorIphone.includes('onAuthStateChanged')&&gestorIphone.includes("collection('usuarios')")&&gestorIphone.includes("collection('usuariosExcluidos')"),evidencia:{aprovada:'Hotfix do iPhone, autenticação, usuários e exclusão definitiva localizados',falha:'A correção específica do iPhone está ausente ou incompleta'},recomendacao:'Restaurar o carregamento do user-management-iphone-fix.js e suas proteções de autenticação e exclusão.'});
