(function(){
  ...
  function posicionarFerramentas(){
    const barra=document.getElementById('firebaseUserBar');
    const sair=document.getElementById('btnSairFirebase');
    const tools=document.getElementById('scpSecurityTools');
    if(!barra||!sair||!tools)return false;
    if(tools.parentElement!==barra||tools.nextElementSibling!==sair)barra.insertBefore(tools,sair);
    return true;
  }
  ...
  function criarBotao(){
    ...
    let tools=document.getElementById('scpSecurityTools');
    if(!tools){
      tools=document.createElement('div');tools.id='scpSecurityTools';tools.className='scp-security-tools';tools.innerHTML='<button type="button" class="scp-security-button" id="scpSecurityButton" aria-label="Segurança" title="Segurança">⚙️</button>';
      barra.appendChild(tools);
      document.getElementById('scpSecurityButton')?.addEventListener('click',abrirPainel);
    }
    posicionarFerramentas();
    window.dispatchEvent(new CustomEvent('scp-security-tools-ready'));
  }
